...
select * from books where extract(year from published_year) between 2020 and 2023;
...
select * from authors where country in ('Germany', 'France') order by first_name, last_name;
-- 7.Романдан башка жана баасы 500 дон кичине болгон китептердин аты, олкосу, чыккан жылы, баасы жанры чыксын..
select books.name, books.country, books.published_year, price, genre  from books where genre not in ('ROMANS') and price < 500;
...
select * from authors where first_name like '____' and email like '%.com' and gender in ('Female');
...
select country from authors group by country having count(country) = 3 order by country;
-- 12. Ар бир жанрдагы китептердин жалпы суммасын чыгарыныз
select sum(books.price), genre from books group by genre;
-- 13. Роман жана Детектив китептеринин эн арзан бааларын чыгарыныз
select genre, min(price) from books group by genre having genre in ('ROMANS', 'DETECTIVE');
-- 14.История жана Биографиялык китептердин сандарын чыгарыныз
select count(*), genre from books group by genre having genre in ('HISTORY', 'BIOGRAPHY');
...
select a.*, p.name as publishers from books
                                          right join authors a on books.author_id = a.id
...
select a.first_name, a.last_name, b.name as book_name
from authors a
         left join books b on a.id = b.author_id;
-- 18.Кайсы тилде канча китеп бар экендиги ылдыйдан ойлдого сорттолуп чыксын.
select l.language, count(b.id) total_book from books b
                                                   inner join languages l on b.language_id = l.id
group by l.language order by total_book desc;
-- 19.Издательствонун аттары жана алардын тапкан акчасынын оточо суммасы тегеректелип чыгарылсын.
select p.name as publishers, round(avg(b.price)) as "rounded sum" from books b
                                                                           inner join publishers p on b.publisher_id = p.id
group by p.name;
-- 20.2010-2015 жылдардын арасындагы китептер жана автордун аты-фамилиясы чыксын.
select a.first_name, a.last_name, b.*  from books b
...
select a.id, a.first_name, a.last_name, sum(b.price) from authors a
                                                              inner join books b on a.id = b.author_id
where extract(year from b.published_year) between 2010 and 2015
group by a.first_name, last_name, a.id;